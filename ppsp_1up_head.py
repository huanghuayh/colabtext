...
# ============================================================
# PPSP_withFundamental (frozen PPSP + dense fundamental head)
# ============================================================
class PPSP_withFundamental(nn.Module):
    def __init__(self, pretrained_ppsp, freeze=True, hidden=256):
        ...
        self.freeze = freeze  # store flag for later use

        if freeze:
            for p in self.harm_net.parameters():
                p.requires_grad = False
            print("[INFO] Frozen PPSP backbone weights ✅")
...
# backbone = PPSP(in_channels=1, out_channels=32)
# backbone.load_state_dict(torch.load("best_ppsp_model_gaussian_[0].pth", map_location="cpu"))
#
